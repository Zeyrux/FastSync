...
|--------|-------|-------------|
| ✅ Parity | 120 | Reproduces rsync's semantics for this option's scope |
...
**Audit cycle (no wire change; `PROTOCOL_VERSION` stays 2.28.0).** A security-and-correctness audit pass ran against the parity-2.29 baseline; none of the fixes changes a row's classification, so the matrix stays **120 ✅ / 10 ⚠️ / 27 ❌ = 157**. The affected rows (`-z`/`--compress`, `--bwlimit`, `-T`/`--temp-dir`, `-p`/`--chmod`, `--partial-dir`, `--filter`) had their notes updated in place:
...
- **`--partial-dir` implies `--partial`.** Matching rsync 3.4.1 (which sets `keep_partial` after option parsing), `--partial-dir=DIR` alone now retains an interrupted transfer's partial and wins over an explicit `--no-partial`; `--inplace` still bypasses the partial machinery.
- **Filter modifiers.** The `x` xattr-name modifier and the merge-only `e`/`n`/`w` modifiers are now rejected with a clear error instead of being silently ignored (`x` on merge/dir-merge rules) or folded into the pattern (producing misleading merge-file errors). Glued patterns (`-newfile`, `-e2e`) keep their historical parsing.
...
`x` xattr-name and merge-only `e`/`n`/`w` modifiers are explicitly rejected, not
silently accepted), receiver-side
...
- **Filter grammar:** `merge`/`.`, `dir-merge`/`:`, `hide`/`H`, `show`/`S`,
  ...
  `.rsync-filter` and `-FF` excludes it. The xattr-name `x` modifier and the
  merge-only `e`/`n`/`w` modifiers are **not implemented** and are rejected with
  a clear error (audit cycle) instead of being silently ignored or folded into
  the pattern.
